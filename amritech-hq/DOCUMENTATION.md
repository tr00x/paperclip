...
> Версия: HQ v2 | Дата: 2026-03-22 | Автор: Tim (@tr00x) + Claude Opus
...
| Email | IONOS SMTP/IMAP | — | Отправка/приём email через agent@amritech.us |
...
| Tunnel | Cloudflare | — | Удалённый доступ (crm.amritech.us, dispatch.amritech.us) |
...
┌─────────────────────────────────────────────────────────────────┐
│                        TELEGRAM GROUP                           │
│  Berik (@ikberik) | Ula (@UlaAmri) | Tim (@tr00x)              │
...
│   └── Closer → готовит briefing → Berik звонит
├── Revenue:
...
└── Люди:
    ├── Berik — решения, звонки, подписи
    ├── Ula — звонки клиентам, collection, on-site
...
| **Telegram** | Агенты пишут в общий чат. Люди видят и реагируют. | CEO шлёт дайджест → Berik одобряет → SDR продолжает |
...
```
Условие: Berik подтвердил closed_won
Действие:
...
**Ключевые концепции:**
- **Company** — AmriTech (id: `b51fd9ff-23e1-44cb-81d3-0238aa9be76c`)
...
DISCOVERY → QUALIFICATION → OUTREACH → ENGAGEMENT → MEETING → CLOSE → ONBOARD → REVENUE
  Hunter      Hunter/CRM      SDR         SDR        Closer    Berik   Onboard   Finance
                                                     +Berik            +Contract
```
...
| engaged | replied_interested | Положительный ответ | Berik решает |
| engaged | replied_question | Вопрос от лида | SDR готовит ответ |
| engaged | replied_objection | Возражение | SDR обрабатывает |
| meeting_set | meeting_scheduled | Звонок назначен | Closer + Berik |
...
- **Подтверждение:** SDR спрашивает Berik перед первичными email'ами
- **Follow-ups Day 3/7:** Автоматом в рабочие часы без подтверждения
- **BCC обязательно:** tr00x@proton.me, ikberik@gmail.com, ula.amri@icloud.com
...
### 7.2 SDR → Berik (ответы лидов)

| Прошло | Tier | Сообщение |
|--------|------|----------|
| 0-2ч | 1 | Обычное уведомление |
| 2-4ч | 2 | "📧 @ikberik, нужно решение — отвечаем?" |
| 4-8ч | 3 | "⚠️ @ikberik, лид остывает!" |
| 8+ч | 4 | "🔴 @ikberik @tr00x СРОЧНО!" |

### 7.3 Contract Manager → Ula (renewals)
...
| Проблема | Demand |
|----------|--------|
| Berik не внёс клиентов | "📋 @ikberik, в CRM {N} клиентов без данных" |
| Ula не записал звонок | "📋 @UlaAmri, нет записи в CRM после звонка" |
...
- Closer → Berik: "Briefing готов 2 дня. Позвонишь?"
...
- `crm.amritech.us` и `dispatch.amritech.us` через named tunnel
- Cloudflare Access: вход по email + одноразовый код
- Допущенные email'ы: ikberik@gmail.com, ula.amri@icloud.com, tr00x@proton.me
...
- **Клиент выбирает время → Berik получает приглашение в календарь**
